...
from hex.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps
from hex.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from hex.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from hex.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_all_steps(self, static=True) -> list[tuple[int, int]]:
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            ...
            if self.delete_pause_frame: # TODO: Debug with EAI data
    ...
    def _get_position_and_gripper_values(self, data: pd.DataFrame) -> tuple[list, list]:
        ...
        # ------------------------------------------------------------------
        # 1) Prefer the new action schema:
        #    action.right_arm  -> use the first 3 dimensions as xyz position
        #    action.right_hand -> use the first dimension as gripper value
        # ------------------------------------------------------------------
        try:
            # Extract xyz position from right_arm[:, :3].
            if "action.right_arm" in action_keys and "right_arm" in le_action_cfg:
                arm_cfg = le_action_cfg["right_arm"]
                arm_key = getattr(arm_cfg, "original_key", None) or "right_arm"

                # The DataFrame column may use either the original key or the full action key.
                if arm_key in data.columns:
                    arm_array = np.stack(data[arm_key])  # [N, D_arm]
                elif "action.right_arm" in data.columns:
                    arm_array = np.stack(data["action.right_arm"])
                else:
                    arm_array = None

                if arm_array is not None:
                    if arm_array.shape[1] < 3:
                        raise ValueError(f"right_arm data has invalid shape: {arm_array.shape}")
                    delta_position_values = arm_array[:, 0:3].tolist()

            # Extract gripper value from right_hand[:, 0].
            if "action.right_hand" in action_keys and "right_hand" in le_action_cfg:
                hand_cfg = le_action_cfg["right_hand"]
                hand_key = getattr(hand_cfg, "original_key", None) or "right_hand"

                # The DataFrame column may use either the original key or the full action key.
                if hand_key in data.columns:
                    hand_array = np.stack(data[hand_key])  # [N, D_hand]
                elif "action.right_hand" in data.columns:
                    hand_array = np.stack(data["action.right_hand"])
                else:
                    hand_array = None

                if hand_array is not None:
                    if hand_array.shape[1] < 1:
                        raise ValueError(f"right_hand data has invalid shape: {hand_array.shape}")
                    gripper_values = hand_array[:, 0].tolist()

        except Exception:
            # Silently fall back to legacy schemas below.
            pass

        # ------------------------------------------------------------------
        # 2) Legacy position formats:
        #    - action.delta_eef_position
        #    - separate action.x / action.y / action.z columns
        # ------------------------------------------------------------------
        if delta_position_values is None:
            position_candidates = ["delta_eef_position"]
            coordinate_candidates = ["x", "y", "z"]

            # First try a combined end-effector position field.
            for pos_key in position_candidates:
                full_key = f"action.{pos_key}"
                if full_key in action_keys:
                    try:
                        if pos_key in le_action_cfg:
                            le_key = getattr(le_action_cfg[pos_key], "original_key", None) or pos_key
                            if le_key in data.columns:
                                data_array = np.stack(data[le_key])
                                le_indices = np.arange(
                                    le_action_cfg[pos_key].start,
                                    le_action_cfg[pos_key].end,
                                )
            ...
            # Then try separate x, y, and z action fields.
            if delta_position_values is None:
                    ...
                    if full_key in action_keys:
                        try:
                            if coord in le_action_cfg:
                                le_key = getattr(le_action_cfg[coord], "original_key", None) or coord
                                if le_key in data.columns:
                                    data_array = np.stack(data[le_key])
                                    le_indices = np.arange(
                                        le_action_cfg[coord].start,
                                        le_action_cfg[coord].end,
                                    )
                ...
                if full_key in action_keys:
                    try:
                        if grip_key in le_action_cfg:
                            le_key = getattr(le_action_cfg[grip_key], "original_key", None) or grip_key
                            if le_key in data.columns:
                                data_array = np.stack(data[le_key])
                                le_indices = np.arange(
                                    le_action_cfg[grip_key].start,
                                    le_action_cfg[grip_key].end,
                                )
        ...
        # ------------------------------------------------------------------
        # 4) Final hard-coded fallback using raw DataFrame column names.
        #    This branch is used when modality metadata is incomplete or mismatched.
        # ------------------------------------------------------------------
        if delta_position_values is None:
            if "action.right_arm" in data.columns:
                arm_array = np.stack(data["action.right_arm"])
                delta_position_values = arm_array[:, 0:3].tolist()
            elif "action.delta_eef_position" in data.columns:
                ...
                raise ValueError(
                    f"No suitable position columns found. Available columns: {data.columns.tolist()}"
                )

        if gripper_values is None:
            if "action.right_hand" in data.columns:
                hand_array = np.stack(data["action.right_hand"])
                gripper_values = hand_array[:, 0].tolist()
            elif "action.gripper_close" in data.columns:
                ...
                raise ValueError(
                    f"No suitable gripper columns found. Available columns: {data.columns.tolist()}"
                )
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        data = self.get_step_data(trajectory_id, base_index, self.vision_history_length, self.action_chunk_size)
    ...
    def get_step_data(self, trajectory_id: int, base_index: int, 
                      vision_history_length: int = 0, action_chunk_size: int = 50) -> dict:
        ...
        for modality in self.modality_keys:
            ...
            for key in self.modality_keys[modality]:
                data[key] = self.get_data_by_modality(trajectory_id, modality, key, base_index, vision_history_length, action_chunk_size)
        return data
    ...
    def get_video(
        ...
        """
        step_indices = self.delta_indices[key] + base_index     # Get the step indices
        trajectory_index = self.get_trajectory_index(trajectory_id)     # Get the trajectory index
        ...
        all_step_indices = []
        for h in reversed(range(vision_history_length + 1)):
            if h == 0:
                idx = step_indices
            else:
                idx = step_indices - h * offset

            idx = np.clip(idx, 0, max_idx)
            all_step_indices.append(idx)
        all_step_indices = np.concatenate(all_step_indices, axis=0)
        ...
        video_timestamp = timestamps[all_step_indices]
    ...
    def get_data_by_modality(
        ...
        """
        if modality == "video":
            return self.get_video(trajectory_id, key, base_index, vision_history_length, action_chunk_size)
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = dataset.transforms(dataset.get_step_data(trajectory_name, step, self.vision_history_length, self.action_chunk_size))
                ...
                if self.need_state:
                    state = []
                    for action_key in dataset.modality_keys["state"]:
                        state.append(data[action_key])
                    state = np.concatenate(state, axis=1).astype(np.float16)
                    sample["state"] = state
                
                # Get tag
                if self.need_tag:
                    sample["tag"] = dataset.tag
