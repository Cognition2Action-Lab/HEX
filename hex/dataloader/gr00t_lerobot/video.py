...
def get_frames_by_timestamps(
    ...
    if video_backend == "decord":
        ...
        for ts in timestamps:
            frame, _ = read_one_frame_at(reader, ts)
            if frame is not None:
                loaded_frames.append(frame)
            else:
                raise RuntimeError(f"Failed to load frame at timestamp {ts}")

        reader.container.close()
        del reader

        frames = np.stack(loaded_frames, axis=0)  # [T, C, H, W]
        return frames.transpose(0, 2, 3, 1)       # [T, H, W, C]
    else:
        raise NotImplementedError


def read_one_frame_at(reader, ts):
    """
    Seek to the nearest keyframe before the target timestamp, then return
    the first decoded frame whose pts is greater than or equal to ts.
    """
    reader.seek(ts, keyframes_only=True)
    for frame in reader:
        current_ts = frame["pts"]
        if current_ts >= ts:
            return np.array(frame["data"]), current_ts
    return None, None
