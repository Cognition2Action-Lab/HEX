...
from hex.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT, SelfAttentionTransformer
from hex.utils.mee import simple_mee_loss_chunk
...
class LayerwiseFlowmatchingActionHead(nn.Module):
    ...
    def forward(self, vl_embs_list: list, actions: torch.Tensor, state: torch.Tensor = None):
        ...
        if self.enable_mee:
            mee_loss = simple_mee_loss_chunk(pred_actions, velocity)
            output_dict = {
                "action_loss": loss,
                "mee_loss": mee_loss,
            }
