...
class _QWen3_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        if config.framework.qwenvl.add_query:
            self.query_token = "<history>"
            self.num_query_tokens = 8
            special_tokens_dict = {"additional_special_tokens": [self.query_token]}
            self.processor.tokenizer.add_special_tokens(special_tokens_dict)
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        if self.config.framework.qwenvl.add_query:
            ...
            attention_mask = batch_inputs["attention_mask"]  # [B, L]
            B, L = input_ids.shape
            q_id = self.processor.tokenizer.convert_tokens_to_ids(self.query_token)
