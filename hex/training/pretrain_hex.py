...
from hex.training.trainer_utils.trainer_tools import TrainerUtils, normalize_dotlist_args, build_param_lr_groups
...
class VLATrainer(TrainerUtils):
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        if self.accelerator.is_main_process:
            ...
            output_dict = self.model.predict_action_batch(
                batch_images=batch_images, instructions=instructions, state=states, tags=tags
            )
    ...
    def pad_actions_and_mask(self, actions, max_action_dim=64):
        B = len(actions)
        T = actions[0].shape[0]
